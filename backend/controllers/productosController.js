  const { productos, insumos } = require('../mockData');
  ...
  //devuelve los insumos de un producto segun la id
  exports.getProductoInsumos = (req, res) => {
    const { id } = req.params;
    const producto = productos.find(p => p.id == id);
    if (producto) {
      const productoInsumos = insumos.filter(i => producto.insumos.includes(i.id));
